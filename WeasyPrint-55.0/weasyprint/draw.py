...
dump_file = os.getenv("DUMP_BOUNDING_BOX") 
dump_path = os.getenv("DUMP_PATH") if os.getenv("DUMP_PATH") else os.path.join(os.path.expanduser("~"),"docgen")
...
def draw_page(page, stream):
    ...
    bbox = draw_stacking_context(stream, stacking_context)
    return bbox
...
def draw_stacking_context(stream, stacking_context):
    ...
    with stacked(stream):
        ...
        if isinstance(box, (boxes.BlockBox, boxes.MarginBox,
            ...
            bbox.append(draw_box_background_and_border(stream, stacking_context.page, box))
        ...
        with stacked(stream):
            ...
            for child_context in stacking_context.negative_z_contexts:
                bbox.append(draw_stacking_context(stream, child_context))

            # Point 4
            for block in stacking_context.block_level_boxes:
                bbox.append(draw_box_background_and_border(
            ...
            if isinstance(box, boxes.InlineBox):
                bbox.append(draw_inline_level(stream, stacking_context.page, box))
...
def draw_inline_level(stream, page, box, offset_x=0, text_overflow='clip',
    ...
    if isinstance(box, StackingContext):
        ...
        if isinstance(box, (boxes.InlineBox, boxes.LineBox)):
            ...
            bbox.append(draw_text(stream, box, offset_x, text_overflow))
...
def draw_first_line(stream, textbox, text_overflow, block_ellipsis, x, y,
    ...
    print("status:", textbox.position_x, textbox.position_y, textbox.width, textbox.height)

    #TODO: Replace with recursion return
    if dump_file:
        os.makedirs(dump_path, exist_ok=True)
        with open(os.path.join(dump_path, 'test.txt'), 'a') as file:
            file.writelines("{},{},{},{}\n".format(textbox.position_x, textbox.position_y, textbox.width, textbox.height))

    return emojis, (textbox.position_x, textbox.position_y, textbox.width, textbox.height)
